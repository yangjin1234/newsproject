...
<li id="menu-item-2486" class="menu-item menu-item-type-custom menu-item-object-custom menu-item-2486"><a href="admin_notfiy.jsp">发放公告</a></li>
<li id="menu-item-2486" class="menu-item menu-item-type-custom menu-item-object-custom menu-item-2486"><a href="admin_manageruser.jsp">返回</a></li>
	...
	<%
	   ...
	   MyLog.log.debug("aaaaa");
	  
	  // if(!request.getParameter("news").equals(""))
	   //{
	   String news=request.getParameter("news");
	   int uid=Integer.parseInt(news);
	   MyLog.log.debug("第一次的uid"+uid);
	   //}
	   //else
	   //{
	  // int uid=Integer.parseInt((String)session.getAttribute("uid"));
	  // MyLog.log.debug("uidaaaaa=="+uid);
	   //}
	...
	LoginDao ld=new LoginDaoImpl();
	 ...
	 List<TimeTypec> list3=nd.selectAllByTime(conn, uid);
	 MyLog.log.debug(list3.toString());
	 request.setAttribute("timetype", list3);
	//查出待发放的工资
	double nosalary=nd.selectSumSalaryNo(uid, conn,0);
	//查出已发放的工资
	double yesalary=nd.selectSumSalaryNo(uid, conn,1);
	...
	int maxPage=nd.getMaxPageByUid(conn, maxSize, uid);
	...
	pageContext.setAttribute("pageNo", pageNo);
	  ...
	  if(typeName==null||typeName.equals("")){
	   list=nd.selectAllNews(uid, conn, pageNo, maxSize);
	   MyLog.log.debug("pageNo="+pageNo);
	   MyLog.log.debug("maxSize="+maxSize);
	   MyLog.log.debug("list="+list.size());
	  }else {
	 // if(!"".equals(typeName)){
	  list=nd.selectNewsByTypeName(uid, conn, typeName, pageNo, maxSize);
	  maxPage=nd.selectNewsByTypeNameForMaxSize(uid, conn, typeName,maxSize);
	...
	//  按时间分类
	  ...
	  maxPage=nd.selectNewsByTimeForMaxSize(uid, conn, timetypecName,maxSize);
	  list=nd.selectNewsByTime(uid, conn, timetypecName, pageNo, maxSize);
	...
	<li class="cat-item cat-item-173"><a href="admin_showarticle.jsp?typec=${typec.tname }" >${typec.tname }</a> (${typec.number })
</li>
	</c:forEach>
</ul>
	...
	<li><a href='admin_showarticle.jsp?timetypec=${timetypec.timec }'>${timetypec.timec}</a>&nbsp;(${timetypec.number })</li>
	</c:forEach></ul>
...
<a class="next page-numbers" href="Newsproject.jsp">下一页</a> -->
 ...
 当前是第${pageNo }页&nbsp;&nbsp;总共有${maxPage }页&nbsp;&nbsp;
   <a href="admin_showarticle.jsp?pageNo=${1 }&pageSize=${maxSize }&typec=${typec }&timetypec=${timetypecName}&news=${uid}"> 首页</a>&nbsp;&nbsp;
    <c:if test="${pageNo ge 2 }">
   <a href="admin_showarticle.jsp?pageNo=${pageNo-1 }&pageSize=${maxSize }&typec=${typec }&timetypec=${timetypecName}&news=${uid}"> 上一页</a>
   </c:if>
   
   <c:forEach begin="1" end="${maxPage }" var="i">
      <c:if test="${i ge pageNo-2 and i le pageNo+2 }">
        <a class='page-numbers' href="admin_showarticle.jsp?pageNo=${i }&pageSize=${maxSize }&typec=${typec }&timetypec=${timetypecName}&news=${uid}"><span class="meta-nav screen-reader-text">页 </span>${i }</a>&nbsp;&nbsp;
      </c:if>
   </c:forEach>
   
   <c:if test="${pageNo le maxPage-1 }">
   <a href="admin_showarticle.jsp?pageNo=${pageNo+1 }&pageSize=${maxSize }&typec=${typec }&timetypec=${timetypecName}&news=${uid}"> 下一页</a>&nbsp;&nbsp;
   </c:if>
   <a href="admin_showarticle.jsp?pageNo=${maxPage }&pageSize=${maxSize }&typec=${typec }&timetypec=${timetypecName}&news=${uid}"> 尾页</a>&nbsp;&nbsp;
