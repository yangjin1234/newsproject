...
public class NewsDaoImpl implements NewsDao{
	...
	public boolean updateNews(News n, Connection conn) throws Exception {
		...
		if(m>0){
			MyLog.log.debug("updateNews修改成功");
			return true;
		}
		return false;
	}
	 ...
	 public List<News> selectNewsMonth(Connection conn,int nid_uid_key,int news_state,int pageNo,int pageSize) throws Exception
	 {
		 ...
		 MyLog.log.debug("进入查询文章的今日的分页数据");
		 String sql="select *from news where nid_uid_key=? and news_state=? and MONTH(namend_time)=? and YEAR(namend_time)=?  limit ?,?";
		 PreparedStatement ps=conn.prepareStatement(sql);
		 ResultSet rs=null;
		 List<News> list=new ArrayList<News>();
		 String nowdate=GetDate.getNowDate();
	 ...
	 {
		 MyLog.log.debug("进入查询文章的今日的最大页数");
		 String sql="select count(*) as max from news where nid_uid_key=? and news_state=? and MONTH(namend_time)=? and YEAR(namend_time)=?";
		 PreparedStatement ps=conn.prepareStatement(sql);
		 String nowdate=GetDate.getNowDate();
		 System.out.println("当前时间为："+nowdate);
		 int a=nowdate.lastIndexOf("-");
	 ...
	 public int selectNewsMonthCount(int nid_uid_key,int news_state,Connection conn) throws Exception
	 {
		 ...
		 String sql="select count(*) as number from news where nid_uid_key=? and news_state=? and MONTH(namend_time)=? and YEAR(namend_time)=? ";
		 PreparedStatement ps=null;
		 ps=conn.prepareStatement(sql);
		 ResultSet rs=null;
		 String nowdate=GetDate.getNowDate();
